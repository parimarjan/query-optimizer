...
// PN: newProject gets its own builder. But createOrdering doesn't...
      RelNode newProject =
...
// PN: find cases this seems to happen.
   ...
   */
  /// best next choice clearly happens here. meat of the stuff.
  ///
...
/// PN: we just make a tree with a single factor, and then pass it back.
//  Next time, will choose between different factors that combine with the
//  current factor to build up the tree?
...
// PN: tree with a single join (or maybe the RelNode we get from the right
// side could also be a join?)
...
/// PN: should we be doing this too? Nope, let RL system learn this.
...
// PN: this is the key point where we updated the RelNode so far.
// Now the RelNode presumably has these two joins added
    relBuilder.push(left.getJoinTree())
        .push(right.getJoinTree())
        .join(joinType, condition);

/// PN: Basically, always applying filters if you can, because should apply
//  them at the earliest anyway.
...
// PN: difference b/w getFactorTree and getJoinTree here?
    return new LoptJoinTree(
        relBuilder.build(),
        left.getFactorTree(),
        right.getFactorTree(),
